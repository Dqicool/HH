...
void selection(const char* infile, const char* outfile)
{
    ...
    if(df.HasColumn("weight_mc"))
    {
        ...
        std::vector<std::string> snap_column = {"bjet_0_pt_scale_fac", "bjet_1_pt_scale_fac", "weight", "gpu_pass", "bjet_0_p4_n", "bjet_1_p4_n", "lep_0_p4_n", "tau_0_p4_n", "met_reco_p4_n"};
        ff.Snapshot("NOMINAL", outfile, snap_column);
    }
    else
    {
        auto ff = ef.Define("weight", []{return (double)1.0;});
        std::vector<std::string> snap_column = {"bjet_0_pt_scale_fac", "bjet_1_pt_scale_fac", "weight"};
        ff.Snapshot("NOMINAL", outfile, snap_column);
    }
}
