...
void plotPropWithCurve(const char* outfile, const char* dist, bool truth){
    ROOT::EnableImplicitMT(24);
    TH1::SetDefaultSumw2();
    //loading data from files
        bool onlyZZ=0;
    ...
    //drawstack
        //sr plot
            ...
            TH1D * h_ratio = new TH1D("ratio","",nbins,xmin,xmax);
            TH1D * h_signif = new TH1D("signif","",nbins,xmin,xmax);
            h_scaled_zz->Scale(1.0/200.0);
            double err;
            for(int i=1; i<nbins; i++)
            {
                double sig_num = h_scaled_zz->Integral(1, i);
                double inc_num = h_inc->IntegralAndError(1,i, err);
                h_ratio->SetBinContent(i,sig_num/inc_num);
                h_signif->SetBinContent(i, sig_num / err);
            }

            TCanvas c2("c2","",IMAGEX,IMAGEY);
                h_ratio->Draw("HIST");
                auto save_name2 = "plots/" + (string)dist + "_ratio.png";
            c2.SaveAs(&save_name2[0]);

            TCanvas c3("c3","",IMAGEX,IMAGEY);
                h_signif->SetAxisRange(0,3,"Y");
                h_signif->Draw("HIST");
                auto save_name3 = "plots/" + (string)dist + "_sigma.png";
            c3.SaveAs(&save_name3[0]);
...
{
    cout<<"mbb:"<<endl;
    plotPropWithCurve("output/05_stack_out/score.root", "score", 0);
